...
# 环境变量：
#   TRANSLATE - 目标语言（如：en, ja, ko等）。如果未设置或为空，则跳过翻译
#   CI_TRANSLATE - CI环境中的目标语言（备用）

input_file="$1"
settings="$2"
output_file="$3"
language="${TRANSLATE=${CI_TRANSLATE}}"

# 检查输入文件参数
if [ -z "${input_file}" ]; then
  log error 未指定输入文件 "filename=${input_file}"
  exit 1
fi
...
if [ -z "${output_file}" ]; then
  ...
  log warn 未指定输出文件，覆盖输入文件 "filename=${input_file},lang=${language}"
fi
...
# 执行翻译
export IS_SANDBOX=1
if claude --dangerously-skip-permissions --print --settings "${settings}" "${translate_prompt}" > "${temp_file}" 2>&1; then
  # 翻译成功，移动临时文件到输出文件
